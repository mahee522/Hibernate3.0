...
public class EmployeeDemo {
	...
	public void demo1(){
		System.out.println("SQL: Joining again in the SQL, resulting in repetation");
		Session session = sf.openSession(); Transaction tx = session.beginTransaction();
		Query query = session.getNamedQuery("emp.native.named.query1");
		List<Employee> result = query.list();
		for (Employee obj: result){
			System.out.println(obj);
			for (Address adr: obj.getAddresses()){
	...
	public void demo3(){
		System.out.println("SQL: Returns one entity using resultSetMapping");
		Session session = sf.openSession(); Transaction tx = session.beginTransaction();
		Query query = session.getNamedQuery("emp.native.named.query3");
		List<Employee> result = query.list();
		for (Employee obj: result){
			System.out.println(obj);
			for (Address adr: obj.getAddresses()){
